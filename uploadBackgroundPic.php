...
if (!isset($_FILES['background_pic']) || $_FILES['background_pic']['error'] !== 0) {
...
if (!move_uploaded_file($_FILES['background_pic']['tmp_name'], $targetPath)) {
  header("Location: profile.php?error=Failed to save file");
  exit();
}

$upd = $conn->prepare("UPDATE profile SET background_pic = ? WHERE user_id = ?");
$upd->bind_param("si", $targetPath, $user_id);
$upd->execute();

header("Location: profile.php?success=Background updated");
exit();